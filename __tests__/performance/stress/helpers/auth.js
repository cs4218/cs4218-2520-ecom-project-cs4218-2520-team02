...
export function getExistingUserCredentials() {
  return {
    email: getOptionalEnv("STRESS_USER_EMAIL") || getRequiredEnv("TEST_USER_EMAIL"),
    password: getOptionalEnv("STRESS_USER_PASSWORD") || getRequiredEnv("TEST_PASSWORD"),
  };
}

export function getStressUserPool() {
  const explicitPool = getJsonEnv("STRESS_USER_POOL", null);
  ...
  const emails = getStringListEnv("STRESS_USER_EMAILS", []);
  const sharedPassword =
    getOptionalEnv("STRESS_USER_PASSWORD") || getOptionalEnv("TEST_PASSWORD");
...
function normalizeUserPoolEntry(entry, index) {
  if (!entry || typeof entry !== "object") {
    throw new Error("Each STRESS_USER_POOL entry must be an object with email and password.");
  }
  ...
  if (!email || !password) {
    throw new Error(
      `Invalid STRESS_USER_POOL entry at index ${index}. Both email and password are required.`
    );
  }
...
export function pickUserForVu(users) {
  if (!Array.isArray(users) || users.length === 0) {
    throw new Error("At least one stress-test user is required.");
  }
...
export function buildUniqueUser() {
  const runId = getOptionalEnv("STRESS_TEST_RUN_ID", "adhoc");
  const suffix = `${Date.now()}-${exec.vu.idInTest}-${exec.scenario.iterationInTest}`;

  return {
    name: `Stress User ${runId} ${suffix}`,
    email: `stress-${runId}-${suffix}@example.com`,
    password: "Stress1234!",
    phone: "91234567",
