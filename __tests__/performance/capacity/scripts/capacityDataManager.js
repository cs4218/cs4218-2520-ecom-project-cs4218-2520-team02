...
function getPoolSize(flow) {
  ...
  const rawValue = process.env[flowKey] || process.env.CAPACITY_USER_POOL_SIZE || "8";
...
function getSeedPassword() {
  return process.env.CAPACITY_SEEDED_USER_PASSWORD || "Capacity1234!";
}

function buildCapacityEmail(runId, flow, index) {
  return `capacity-${runId}-${flow}-seed-${index + 1}@example.com`;
}

function buildCapacityPrefix(runId) {
  return `capacity-${runId}-`;
}
...
function buildCapacityEmailMatcher(runId) {
  if (!runId) {
    return { $regex: /^capacity-/ };
  }

  const prefix = buildCapacityPrefix(runId);
...
function buildUserPool(flow, runId, count) {
  return Array.from({ length: count }, (_, index) => ({
    ...
    email: buildCapacityEmail(runId, flow, index),
    password: getSeedPassword(),
    name: `Capacity ${flow} User ${index + 1}`,
  }));
}
...
async function connectToDatabase() {
  ...
  if (!mongoUrl) {
    throw new Error("MONGO_URL is required to seed capacity-test data.");
  }
...
async function ensureOrderHistory(users) {
  ...
  if (products.length === 0) {
    throw new Error("At least one product is required to seed order-history capacity users.");
  }
  ...
  for (const user of users) {
    await orderModel.create({
      ...
      payment: {
        success: true,
        transaction: {
          id: `capacity-order-${user._id.toString()}`,
        },
      },
      buyer: user._id,
      status: "Delivered",
...
export async function prepareCapacityData(flow, runId) {
...
export async function cleanupCapacityData(runId) {
  await connectToDatabase();

  const users = await userModel.find({ email: buildCapacityEmailMatcher(runId) }).select("_id email");
...
export async function disconnectCapacityDatabase() {
