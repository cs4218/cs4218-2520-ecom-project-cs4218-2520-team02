 ...
 *
 * Verifies that the checkout and payment system performs reliably under
 * sustained and realistic user traffic. Simulates steady concurrent users
 * performing checkout and payment submissions.
 *
 * Performance criteria:
 *   - P95 response time remains consistent under sustained load
 *   - Failed payment rate stays within acceptable thresholds (< 5%)
 */

const baseUrl = getBaseUrl();
let cachedSession = null;

export const options = createLoadOptions({ flow: "payment" });

export function setup() {
  const users = getLoadUserPool();
  if (users.length === 0) {
    throw new Error("Payment load test requires at least one valid user.");
  }
  ...
  if (validUsers.length === 0) {
    throw new Error("Payment load test requires at least one user with valid login credentials.");
  }
  ...
  if (products.length === 0) {
    throw new Error("The payment load test needs at least one product in the catalogue.");
  }
