...
export function getExistingUserCredentials() {
  return {
    email: getOptionalEnv("LOAD_USER_EMAIL") || getRequiredEnv("TEST_USER_EMAIL"),
    password: getOptionalEnv("LOAD_USER_PASSWORD") || getRequiredEnv("TEST_PASSWORD"),
  };
}

export function getLoadUserPool() {
  const explicitPool = getJsonEnv("LOAD_USER_POOL", null);
  ...
  const emails = getStringListEnv("LOAD_USER_EMAILS", []);
  const sharedPassword =
    getOptionalEnv("LOAD_USER_PASSWORD") || getOptionalEnv("TEST_PASSWORD");
...
function normalizeUserPoolEntry(entry, index) {
  if (!entry || typeof entry !== "object") {
    throw new Error("Each LOAD_USER_POOL entry must be an object with email and password.");
  }
  ...
  if (!email || !password) {
    throw new Error(
      `Invalid LOAD_USER_POOL entry at index ${index}. Both email and password are required.`
    );
  }
...
export function pickUserForVu(users) {
  if (!Array.isArray(users) || users.length === 0) {
    throw new Error("At least one load-test user is required.");
  }
...
export function buildUniqueUser() {
  const runId = getOptionalEnv("LOAD_TEST_RUN_ID", "adhoc");
  const suffix = `${Date.now()}-${exec.vu.idInTest}-${exec.scenario.iterationInTest}`;

  return {
    name: `Load User ${runId} ${suffix}`,
    email: `load-${runId}-${suffix}@example.com`,
    password: "Load1234!",
    phone: "91234567",
