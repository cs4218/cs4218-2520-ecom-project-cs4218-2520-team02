  ...
  const SECRET_PATTERNS = [
    {
      ...
      // Matches: jwt.sign(payload, "someSecret") means process.env references not used
      regex:
        /jwt\.(sign|verify)\s*\([^)]*,\s*["'`][A-Za-z0-9+/=_\-!@#$%^&*]{8,}["'`]/g,
    },
    {
    ...
    },
    {
      ...
      regex:
        /(merchantId|publicKey|privateKey|merchant_id|public_key|private_key)\s*[:=]\s*["'`][A-Za-z0-9]{10,}["'`]/g,
    },
    {
      name: "Hardcoded password assignment",
      // Matches: password = "literal" or password: "literal" means process.env not used
    ...
    },
    {
      ...
      regex:
        /\b(api[_-]?key|secret[_-]?key|access[_-]?token|auth[_-]?token)\s*[:=]\s*["'`][A-Za-z0-9+/=-]{20,}["'`]/gi,
    },
  ];
  ...
  it("should not use dangerouslySetInnerHTML in React components", () => {
    ...
    for (const filePath of getSourceFiles()) {
      ...
      lines.forEach((line, idx) => {
        const trimmed = line.trim();
        if (
          /dangerouslySetInnerHTML/.test(trimmed) &&
          !trimmed.startsWith("//") &&
          !trimmed.startsWith("*")
        ) {
          findings.push(`${relPath}:${idx + 1} → ${trimmed.slice(0, 100)}`);
...
describe("Security: Dependency Vulnerabilities", () => {
  it("should have no critical severity vulnerabilities in dependencies", () => {
    let auditResult;
    try {
      execSync("npm audit --json --audit-level=critical", {
        cwd: ROOT,
        encoding: "utf8",
      });
