 ...
 *
 * Verifies that all protected API endpoints enforce server-side ownership
 * checks and role-based access control, preventing:
 *
 *   - Unauthenticated access to protected routes
 *   - Privilege escalation (regular user accessing admin-only routes)
 *   - Cross-user data leakage (User A reading User B's orders/profile)
 *   - Token manipulation (tampered signature, fabricated payload, expired)
 *   - Profile IDOR (supplying another user's ID in the request body)
 *
 * Approach: Direct HTTP with crafted Authorization headers. Two real test
 * users are registered and logged in during setup; admin-route tests use the
 * regular user token to verify rejection.
 *
 * Prerequisites:
 *   npm run server   (app must be running at APP_URL)
 *
 * Run: npm run test:idor
 *
 * Security Metrics tracked:
 *   - Number of endpoints vulnerable to unauthorised resource access
 *   - Number of IDOR attempts that return 200 with another user's data
 */
...
async function registerAndLogin(user) {
  // Register (ignore 200 "already registered" — may exist from a prior run)
  ...
  if (!loginRes.body?.token) {
    throw new Error(`Setup: could not login ${user.email} — ${loginRes.raw.slice(0, 200)}`);
  }
...
describe('IDOR: Unauthenticated Access', () => {
  ...
  it('PUT /order-status/:id (admin) should return 401 with no token', async () => {
    if (skipIfDown()) return;
    const res = await put(`/api/v1/auth/order-status/${FAKE_OBJECT_ID}`, { status: 'Shipped' });
    expect(res.status).toBe(401);
  });
  ...
  it('DELETE /product/:pid (admin) should return 401 with no token', async () => {
    if (skipIfDown()) return;
    const res = await request('DELETE', `/api/v1/product/delete-product/${FAKE_OBJECT_ID}`);
    expect(res.status).toBe(401);
  });
});
...
describe('IDOR: Privilege Escalation — Admin Routes with User Token', () => {
  ...
  it('DELETE /product/:pid should return 403 for regular user', async () => {
    if (skipIfDown()) return;
    const res = await request('DELETE', `/api/v1/product/delete-product/${FAKE_OBJECT_ID}`, null, tokenA);
    expect(res.status).toBe(403);
  });
...
describe('IDOR: Order Data Isolation Between Users', () => {
  ...
  it('GET /orders response must only contain orders where buyer matches the token identity', async () => {
    ...
    for (const order of orders) {
      ...
      // buyer is populated with "name" only — verify it matches USER_A
      if (order.buyer?.name) {
...
describe('IDOR: Token Manipulation', () => {
  it('tampered JWT signature should return 401', async () => {
    if (skipIfDown()) return;
    // Take User A's valid token and corrupt the signature segment
  ...
  it('JWT signed with wrong secret should return 401', async () => {
    if (skipIfDown()) return;
    const fakeToken = JWT.sign({ _id: FAKE_OBJECT_ID, role: 0 }, 'wrong-secret-key', { expiresIn: '1d' });
  ...
  it('JWT with fabricated non-existent user ID should be rejected on admin routes', async () => {
    ...
    // isAdmin does findById → null → 401 "User not found"
    ...
    // Must not return 200 — either 401 (user not found) or 403
  ...
  it('empty Bearer token should return 401', async () => {
    if (skipIfDown()) return;
    const res = await get('/api/v1/auth/orders', '');
    // Empty string means no Authorization header reaches server
    expect(res.status).toBe(401);
  });
});
