...
if (!ZAP_API_KEY)
  throw new Error(
    "ZAP_API_KEY environment variable is required. Set it in .env or pass it inline: ZAP_API_KEY=<key> npm run test:dast",
  );
...
const FALSE_POSITIVES = [
  {
    ...
    // leakage. Mongoose passes the email field as a typed BSON string so SQL
  ...
  },
  {
    ...
    reason:
      "CSP default-src 'none' is intentionally strict (API server serves no browser resources). All no-fallback directives are explicitly set in helmet config.",
  },
];

function isFalsePositive(alert) {
  return FALSE_POSITIVES.some(
    (fp) => fp.alertPattern.test(alert.alert) && fp.condition(alert),
  );
}

function falsePositiveReason(alert) {
  return (
    FALSE_POSITIVES.find(
      (fp) => fp.alertPattern.test(alert.alert) && fp.condition(alert),
    )?.reason ?? ""
  );
}
      ...
      .get(url, (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            reject(
              new Error(`ZAP response not valid JSON: ${data.slice(0, 200)}`),
            );
          }
        });
      })
...
async function checkConnectivity() {
  ...
  // Check app by using the local URL since this check runs on the host, not inside Docker
  try {
    await new Promise((resolve, reject) => {
      http
        .get(`${APP_LOCAL_BASE_URL}/`, (res) => {
          res.resume();
          resolve();
        })
        .on("error", reject);
...
async function seedEndpoints() {
  const seeds = [
    ...
    {
      url: AUTH_ENDPOINTS[1], // login with missing fields
      body: JSON.stringify({}),
    },
    {
...
async function runScans() {
  ...
  try {
    await zapPost("context/action/removeContext/", {
      contextName: "auth-dast",
    });
  } catch {
    // Context may not exist on the first run so its safe to ignore
  }
  const ctxResult = await zapPost("context/action/newContext/", {
    contextName: "auth-dast",
  });
  scanContextId = ctxResult.contextId;
  if (!scanContextId)
    throw new Error(
      `Failed to create ZAP context: ${JSON.stringify(ctxResult)}`,
    );
  ...
  // Active scan (sends attack payloads, including SQL injection, XSS, etc.)
...
beforeAll(async () => {
  ...
  if (!zapAvailable || !appAvailable) return;
...
function skipIfUnavailable() {
  if (!appAvailable) {
    console.warn(
      "\n  SKIPPED: Virtual Vault server is not reachable at " +
        APP_LOCAL_BASE_URL +
  ...
  if (!zapAvailable) {
    console.warn(
      "\n  SKIPPED: OWASP ZAP is not reachable at " +
        ZAP_BASE_URL +
        "\n  Start ZAP with:" +
...
describe("DAST: Authentication Endpoint Availability", () => {
  it(
    "Virtual Vault server should be reachable at " + APP_LOCAL_BASE_URL,
    async () => {
      if (!appAvailable) {
        throw new Error(
...
describe("DAST: High Severity Vulnerabilities", () => {
  ...
  it("should have no SQL injection vulnerabilities on login and register", async () => {
    ...
    if (realSqliAlerts.length > 0) {
      const report = realSqliAlerts
        ...
        .join("\n");
      throw new Error(
        `SQL Injection detected on authentication endpoints:\n${report}`,
      );
    }
  });
  ...
  it("should have no command injection vulnerabilities", async () => {
    ...
    const cmdAlerts = allAlerts.filter(
      (a) =>
        /command.inject|remote.os.command/i.test(a.alert) ||
        a.pluginId === "90020",
    );
...
describe("DAST: Information Disclosure", () => {
  it("should not expose sensitive information in error responses", async () => {
    ...
    const disclosureAlerts = allAlerts.filter((a) =>
      /information disclosure|application error|debug error|stack trace|server error information/i.test(
        a.alert,
      ),
    );
...
describe("DAST: Security Headers", () => {
  it("should not be missing critical security response headers", async () => {
    ...
    for (const [header, [alertName, pluginId]] of Object.entries(
      HEADER_ALERTS,
    )) {
  ...
  it("should not return an overly verbose Server header", async () => {
    ...
    const serverHeaderAlert = allAlerts.find(
      (a) =>
        /server leaks version information/i.test(a.alert) ||
        a.pluginId === "10036",
    );
...
describe("DAST: Authentication-Specific Vulnerabilities", () => {
  it("should not be vulnerable to brute-force (no rate limiting detected)", async () => {
    ...
    // ZAP does not directly test rate limiting, hence a manual test here
  ...
  it("should not be vulnerable to XSS via registration input fields", async () => {
    ...
    if (xssAlerts.length > 0) {
      const report = xssAlerts
        ...
        .join("\n\n");
      throw new Error(
        `XSS vulnerabilities detected on authentication endpoints:\n\n${report}`,
      );
    }
  });
});
